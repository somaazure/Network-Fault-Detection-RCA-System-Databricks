...
def setup_enhanced_logging():
    ...
    try:
        ...
        logger.info("✅ Enhanced logging configured - writing to DBFS and console")
...
class DatabricksIncidentManager:
            ...
            for match in matches:
                try:
                    ...
                    if cleaned_match:
                        ...
                        logger.info(f"✅ JSON extracted using regex pattern: {pattern[:20]}...")
                        return parsed_json
                except json.JSONDecodeError:
                    continue
    ...
    def _create_incident_log_file(self, incident_id: str, fm_response: str, parsed_json: Optional[Dict], final_decision: Dict) -> str:
        ...
        try:
            ...
            logger.info(f"📝 Incident log saved: /FileStore/logs/incidents/{incident_id}.log")
            return log_filename
            ...
            **RESPONSE FORMAT:**
            ...
            if model_response["success"]:
                ...
                if parsed_json:
                    ...
                    logger.info("🎯 JSON successfully extracted from FM response")
                    ...
                    logger.warning("🔧 JSON extraction failed, using rule-based analysis")
            ...
            if log_file_path:
                logger.info(f"   • Log File: {log_file_path.replace('/dbfs', '')}")
...
# MAGIC %md
# MAGIC ## 📋 Databricks Log Explorer Widget
# MAGIC
...
def create_log_explorer_widget():
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ Could not list incident logs: {e}")
        
        if not log_files:
            print("📂 No incident log files found yet.")
            print("💡 Run some incident processing first to generate logs.")
            return
        
        print("📋 DATABRICKS LOG EXPLORER")
        print("=" * 50)
        ...
        for i, log_file in enumerate(log_files[:10], 1):  # Show up to 10 most recent
            ...
            print(f"    🔗 View: %fs head {file_path}")
            print(f"    📥 Download: Available in Databricks UI")
            print()
        
        # Show main incident manager log
        print("📊 MAIN INCIDENT MANAGER LOG:")
        print("    📁 Path: /FileStore/logs/incident_manager.log")
        print("    🔗 View: %fs head /FileStore/logs/incident_manager.log")
        print("    📥 Download: Available in Databricks UI")
        print()
        
        # Provide instructions
        print("💡 USAGE INSTRUCTIONS:")
        print("1. Copy any '🔗 View:' command above and run in a new cell")
        ...
        print(f"❌ Error creating log explorer: {e}")
...
print("📋 Log files available in DBFS /FileStore/logs/ directory")
