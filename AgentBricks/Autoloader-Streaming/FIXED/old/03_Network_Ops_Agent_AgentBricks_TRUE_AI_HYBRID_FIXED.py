...
def process_operations_batch(batch_df, batch_id):
    print(f"\nðŸ”§ Processing operations batch {batch_id} with {batch_df.count()} incident decisions")
    ...
    for idx, row in enumerate(rows):
        ...
        incident_data = {
            ...
            "escalation_required": row_dict["escalation_required"],
            "original_log_content": row_dict["original_log_content"]
        }
...
try:
    ...
    if final_ops > 0:
        method_analysis = spark.table(NETWORK_OPS_TABLE).groupBy("planning_method").count().collect()
        ...
        for row in method_analysis:
            print(f"   {row['planning_method']}: {row['count']}")
        ...
        samples = spark.table(NETWORK_OPS_TABLE).select(
            "recommended_operation","risk_level","planning_method","operation_reasoning"
        ).limit(3).collect()
        
        for row in samples:
            print(f"   {row['recommended_operation']} (Risk: {row['risk_level']}) via {row['planning_method']}")
            print(f"     Reasoning: {row['operation_reasoning']}")
    else:
