...
# Multi-Table Join and Orchestration
def get_complete_workflow_data():
    #\"\"\"Join all agent results for complete workflow view\"\"\"
    try:
        # Start with severity classifications as the base
        severity_df = spark.table(SEVERITY_TABLE).alias("sev")
        
        # Try to join with other tables if they exist and have data
        workflow_df = severity_df
        join_count = 1
        
        if spark.catalog.tableExists(INCIDENTS_TABLE):
            incidents_df = spark.table(INCIDENTS_TABLE).alias("inc")
        ...
        if spark.catalog.tableExists(NETWORK_OPS_TABLE):
            ops_df = spark.table(NETWORK_OPS_TABLE).alias("ops")
            if ops_df.count() > 0:
                workflow_df = workflow_df.join(
                    ops_df,
        ...
        if spark.catalog.tableExists(RCA_TABLE):
            rca_df = spark.table(RCA_TABLE).alias("rca")
            if rca_df.count() > 0:
                workflow_df = workflow_df.join(
                    rca_df,
        ...
        print(f"ðŸ“Š Workflow join completed: {join_count} tables joined")
        return final_df
...
def process_orchestrator_batch(batch_df, batch_id):
    ...
    try:
        ...
        if complete_workflows.count() == 0:
            print("âš ï¸ No complete workflow data available yet")
            return
        
        print(f"ðŸ“Š Found {complete_workflows.count()} complete workflows to orchestrate")
