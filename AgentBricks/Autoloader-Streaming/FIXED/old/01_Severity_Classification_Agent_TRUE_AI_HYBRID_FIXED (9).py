...
try:
    ...
    # IMMEDIATE TEST - VERIFY CLIENT WORKS
    test_result = hybrid_classify("CRITICAL test system failure")
    print(f"IMMEDIATE TEST RESULT: {test_result['severity']} via {test_result['method']}")
...
def classify_with_fm(log_content: str) -> dict:
    ...
    global client, AI_ENABLED  # FORCE GLOBAL ACCESS
    
    if not AI_ENABLED:
        print(f"AI_ENABLED is False - skipping FM call")
        return {"success": False}
    
    if client is None:
        print(f"Client is None - skipping FM call")
        return {"success": False}
    
    try:
        print(f"AI CALL for: {log_content[:30]}...")
...
def hybrid_classify(log_content: str) -> dict:
    ...
    print(f"HYBRID_CLASSIFY called for: {log_content[:30]}...")
    
    # Try AI first (guaranteed working pattern)
    fm_res = classify_with_fm(log_content)
    
    if fm_res.get("success"):
        print(f"USING AI CLASSIFICATION: {fm_res.get('severity')}")
        return fm_res
    else:
        print(f"Using rules fallback")
