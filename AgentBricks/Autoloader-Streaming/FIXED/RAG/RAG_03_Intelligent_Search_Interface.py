...
# MAGIC %md
# MAGIC # ðŸ¤– RAG Intelligent Search Interface
# MAGIC
...
print("ðŸ¤– RAG Intelligent Search Interface")
print("=" * 70)
...
class NetworkFaultRAGSearchEngine:
    ...
    def __init__(self):
        ...
        print("ðŸ”§ Initializing NetworkFault RAG Search Engine...")
        if self.vs_client:
            self._validate_setup()
        else:
            print("âš ï¸ Vector Search client not available - using fallback mode")
        ...
        try:
            ...
            for index_type, index_name in self.search_indexes.items():
                try:
                    ...
                    print(f"   âœ… {index_type.title()} index available")
                except Exception:
                    print(f"   âŒ {index_type.title()} index not available")

            self.available_indexes = available_indexes
            print(f"ðŸŽ¯ RAG Engine initialized with {len(available_indexes)} search indexes")

        except Exception as e:
            print(f"âŒ Error validating RAG setup: {str(e)}")
            self.available_indexes = []
    ...
    def determine_search_strategy(self, query: str) -> str:
        ...
        try:
            ...
            print(f"âš ï¸ Error counting keyword matches: {str(e)}")
        ...
        if solution_matches > technical_matches and "solution" in self.available_indexes:
            ...
            if self.available_indexes and len(self.available_indexes) > 0:
                ...
                print("âš ï¸ No available indexes, using comprehensive as fallback")
                return "comprehensive"
    ...
    def search_similar_incidents(self, query: str, num_results: int = 5, strategy: str = None) -> Dict[str, Any]:
        ...
        print(f"ðŸ” Searching with {strategy} strategy using index: {index_name}")
        ...
        try:
            ...
            if search_results and 'result' in search_results and 'data_array' in search_results['result']:
                ...
                print(f"âœ… Found {len(results)} similar incidents")
...
**Technical Response:**"""
            ...
            try:
                ...
                try:
                    ...
                    print(f"âš ï¸ Error parsing AI response: {str(parse_error)}")
                    ai_response = None
...
try:
    print("ðŸš€ Initializing RAG Search Engine...")
    rag_engine = NetworkFaultRAGSearchEngine()
    print(f"âœ… RAG Engine initialized with {len(rag_engine.available_indexes)} available indexes")
except Exception as init_error:
    print(f"âŒ RAG Engine initialization failed: {str(init_error)}")
    print("ðŸ’¡ This may indicate Vector Search setup issues. Run RAG_01 and RAG_02 first.")
    rag_engine = None
...
def demo_intelligent_search():
    ...
    print("ðŸŽ¯ DEMONSTRATING RAG INTELLIGENT SEARCH")
    print("=" * 70)

    # Check if RAG engine is available
    if not rag_engine:
        print("âŒ RAG engine not available. Cannot run demonstration.")
        print("ðŸ’¡ Please run RAG_01 and RAG_02 first to set up Vector Search.")
        return
    ...
    for i, query in enumerate(demo_queries, 1):
        ...
        try:
            ...
            print(f"ðŸ“Š Search Results: {search_info.get('results_count', 0)} similar incidents found")

            if "error" not in rag_info:
                print(f"ðŸ¤– RAG Response Method: {rag_info.get('method', 'unknown')}")
                print(f"ðŸŽ¯ Confidence: {rag_info.get('confidence', 'unknown')}")
                print(f"ðŸ“‹ Sources: {len(rag_info.get('sources', []))} incident references")

                # Show full response
                full_response = rag_info.get('response', 'No response generated')
                print(f"ðŸ’¬ Full RAG Response:")
                print(f"{full_response}")

            else:
                print(f"âŒ RAG Error: {rag_info['error']}")
    ...
    print("ðŸŽ¯ Intelligent search demonstration completed!")
...
def advanced_search_features():
    ...
    print("ðŸš€ ADVANCED SEARCH FEATURES")
    print("=" * 50)

    # Check if RAG engine is available
    if not rag_engine:
        print("âŒ RAG engine not available. Cannot run advanced features.")
        return

    # Multi-strategy search comparison
    test_query = "router BGP protocol failure high priority incident"

    print(f"ðŸ” Comparing search strategies for: '{test_query}'")
    print()
    ...
    for strategy in strategies:
        if strategy in rag_engine.available_indexes:
            print(f"ðŸ“Š {strategy.title()} Strategy:")
            try:
            ...
            print(f"âŒ {strategy.title()} Strategy: Index not available")
...
def display_search_analytics():
    ...
    print("ðŸ“Š SEARCH ANALYTICS")
    print("=" * 50)

    # Check if RAG engine is available
    if not rag_engine:
        print("âŒ RAG engine not available. Cannot display analytics.")
        return

    # Check if rag_engine exists and has search_history attribute
    if not rag_engine or not hasattr(rag_engine, 'search_history'):
        print("â„¹ï¸ No search history available - RAG engine not initialized")
        return

    if not rag_engine.search_history:
        print("â„¹ï¸ No search history available yet")
        print("ðŸ’¡ Run some demo searches first to see analytics")
        return
    ...
    try:
        ...
        print(f"âŒ Error processing search analytics: {str(e)}")
        print(f"ðŸ“Š Search history type: {type(getattr(rag_engine, 'search_history', 'not found'))}")
        print(f"ðŸ“Š Search history length: {len(search_history) if 'search_history' in locals() else 'unknown'}")
        if 'search_history' in locals() and search_history:
            print(f"ðŸ“Š First entry type: {type(search_history[0]) if search_history else 'empty'}")
        return

    print(f"ðŸ” Total Searches: {total_searches}")
    print(f"âœ… Successful Searches: {successful_searches} ({(successful_searches/total_searches)*100:.1f}%)")
    print(f"ðŸ“Š Average Results per Search: {avg_results:.1f}")
    ...
    try:
        ...
        print(f"âŒ Error displaying search methods and history: {str(e)}")
        print(f"ðŸ“Š Available search data: {len(search_history) if 'search_history' in locals() else 0} entries")
...
def custom_search_interface():
    ...
    print("ðŸ’¬ CUSTOM SEARCH INTERFACE")
    print("=" * 50)

    # Check if RAG engine is available
    if not rag_engine:
        print("âŒ RAG engine not available. Cannot run custom search interface.")
        print("ðŸ’¡ Please run RAG_01 and RAG_02 first to set up Vector Search.")
        return
    ...
    for query in custom_queries:
        ...
        try:
            ...
            if "error" not in rag_response:
                ...
                if sources_count > 0:
                    print(f"\nðŸ“š Sources: {sources_count} related incidents")
            else:
                print(f"âŒ Error: {rag_response['error']}")
        except Exception as e:
            print(f"âŒ Custom search failed: {str(e)}")

        print()

    print("âœ… Custom search interface demonstration completed")
...
print("\nðŸŽ¯ RAG INTELLIGENT SEARCH SYSTEM READY!")
print("ðŸ’¡ Use the NetworkFaultRAGSearchEngine class for production integration")
...
print("\nðŸŽ¯ RAG_03 DEPLOYMENT COMPLETE!")
print("âœ… Intelligent Search Interface Ready")
print("ðŸš€ Next step: Run RAG_04_End_to_End_Testing.py")
