...
# MAGIC %md
# MAGIC # 04 Rca Agent Agentbricks True Ai Hybrid Fixed
# MAGIC
...
# Databricks notebook source
# MAGIC %md
# MAGIC # 🚀 Root Cause Analysis Agent - TRUE AI + Rules Hybrid (FIXED)
...
print("🛠️ RCA Agent checkpoint cleanup function ready")
...
try:
    ...
    print(f"📊 Found existing RCA table with {existing_rca} records - will append new data")
except:
    print("📋 Creating new RCA reports table")
    spark.sql(f"""
...
print("✅ RCA reports table ready")

# COMMAND ----------

# ForeachBatch Processor  
def process_rca_batch(batch_df, batch_id):
    print(f"\n🔍 Processing RCA batch {batch_id} with {batch_df.count()} network operations")
    ...
    if results:
        ...
        print(f"✅ Wrote {len(results)} RCA reports")
    else:
        print("⚠️ No results")
...
cleanup_checkpoint_if_needed(RCA_CHECKPOINT, RCA_TABLE, "RCA Analysis")

# Start Streaming (depends on network operations)
print("🌊 Starting RCA streaming...")
...
while time.time() - start_time < duration:
    ...
    try:
        ...
        print(f"⏰ [{elapsed}s/{duration}s] Operations={ops_count}, RCA Reports={rca_count}")
    except:
        print(f"⏰ [{elapsed}s/{duration}s] Monitoring...")
    time.sleep(15)

rca_stream.stop()
print("🛑 RCA stream stopped")
...
try:
    ...
    print(f"🎯 RCA reports generated: {final_rca}")
    ...
    if final_rca > 0:
        ...
        print("❌ No RCA reports generated")
except Exception as e:
