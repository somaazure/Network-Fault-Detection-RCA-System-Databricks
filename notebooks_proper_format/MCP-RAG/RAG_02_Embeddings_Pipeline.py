...
# MAGIC %md
# MAGIC # Rag 02 Embeddings Pipeline
# MAGIC
...
# Databricks notebook source
# MAGIC %md
# MAGIC # 🧠 RAG Implementation - Embeddings Pipeline
# MAGIC
...
print("🧠 RAG Implementation - Embeddings Pipeline")
print("=" * 70)
...
def assess_embedding_readiness():
    ...
    print("📊 Assessing RCA data for embedding quality...")
    ...
    try:
        ...
        print(f"📝 Non-null RCA: {quality_analysis['non_null_rca']:,}")
        print(f"📊 Substantial Content (>100 chars): {quality_analysis['substantial_content']:,}")
        print(f"🎯 Rich Content (>500 chars): {quality_analysis['rich_content']:,}")
        print(f"📏 Average Content Length: {quality_analysis['avg_content_length']:.0f} chars")
        print(f"🔧 Unique Categories: {quality_analysis['unique_categories']}")
        print(f"🚨 Unique Priorities: {quality_analysis['unique_priorities']}")

        # Calculate quality score
        quality_score = (quality_analysis['substantial_content'] / quality_analysis['total_records']) * 100
        print(f"✅ Content Quality Score: {quality_score:.1f}%")

        return quality_analysis

    except Exception as e:
        print(f"❌ Error assessing embedding readiness: {str(e)}")
        return None
...
def create_enhanced_embedding_content():
    ...
    print("🔧 Creating enhanced content for embeddings...")
    ...
    try:
        ...
        print(f"✅ Created enhanced content for {enhanced_count:,} RCA reports")
        ...
        print(f"❌ Error creating enhanced content: {str(e)}")
        return None
...
def create_specialized_vector_indexes():
    ...
    if not VECTOR_SEARCH_AVAILABLE:
        print("⚠️ Vector Search not available - skipping index creation")
        print("💡 Use RAG_Fallback_Basic_Search.py for text-based search instead")
        return False

    if enhanced_content_df is None:
        print("❌ Enhanced content not available")
        return False
    ...
    try:
        ...
        print(f"💾 Enhanced content saved to: {ENHANCED_TABLE}")
        ...
        for config in index_configs:
            try:
                print(f"\n🔧 Creating {config['description']}: {config['name']}")
                ...
                print(f"✅ Created index: {config['name']}")
                created_indexes.append(config['name'])

            except Exception as index_error:
                print(f"❌ Error creating index {config['name']}: {str(index_error)}")

        print(f"\n📊 Successfully created {len(created_indexes)} vector indexes")
        return created_indexes

    except Exception as e:
        print(f"❌ Error creating specialized indexes: {str(e)}")
        return []
...
def validate_embeddings_quality():
    ...
    print("🔍 Validating embeddings quality...")
    ...
    try:
        ...
        for test_query in validation_queries:
            ...
            try:
                ...
                if results and 'result' in results and 'data_array' in results['result']:
                    ...
                    print(f"   ✅ Found {result_count} similar results")
        ...
        if validation_results:
            ...
            print(f"   ✅ Queries with Results: {queries_with_results}/{len(validation_results)}")
            print(f"   📈 Search Success Rate: {(queries_with_results/len(validation_results))*100:.1f}%")

            # Embedding quality assessment
            if avg_relevance >= 70 and queries_with_results >= len(validation_results) * 0.8:
                print("   🎯 Embeddings Quality: ✅ EXCELLENT")
            elif avg_relevance >= 50 and queries_with_results >= len(validation_results) * 0.6:
                print("   🎯 Embeddings Quality: ⚠️ GOOD")
            else:
                print("   🎯 Embeddings Quality: ❌ NEEDS IMPROVEMENT")
...
def setup_incremental_update_pipeline():
    ...
    print("🔄 Setting up incremental update pipeline...")
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Tracking table already exists: {EMBEDDING_TRACKING_TABLE}")
        except:
            empty_tracking_df.write.mode("overwrite").saveAsTable(EMBEDDING_TRACKING_TABLE)
            print(f"✅ Created tracking table: {EMBEDDING_TRACKING_TABLE}")
...
def update_embeddings_incrementally():
    ...
    print(f"🔄 Processing {{new_count}} new RCA records...")
    ...
    try:
        ...
        for index_name in indexes_to_check:
            try:
                ...
                print(f"   📊 Index {index_name} status checked")
            except Exception as check_error:
                print(f"   ⚠️ Could not check index {index_name}: {str(check_error)}")
        ...
        print(f"✅ Incremental update completed: {new_count} records processed")
...
"""
        ...
        print("✅ Incremental update pipeline configured")
        print("💡 Use the generated function to update embeddings for new RCA reports")

        return EMBEDDING_TRACKING_TABLE

    except Exception as e:
        print(f"❌ Error setting up incremental pipeline: {str(e)}")
        return None
...
if created_indexes:
    print(f"\n🔍 Created Vector Indexes ({len(created_indexes)}):")
    for idx in created_indexes:
        print(f"   📊 {idx}")

# Overall pipeline readiness
all_ready = all(pipeline_status.values())
print(f"\n🎯 Embeddings Pipeline Ready: {'✅ YES' if all_ready else '❌ NO'}")
...
print(f"\n📄 Embeddings configuration ready for RAG interface")
print("🎯 Next step: Build RAG query interface")
...
if quality_assessment and validation_results:
    print("\n📊 FINAL STATISTICS")
    print("=" * 50)
    print(f"📋 Total RCA Reports: {quality_assessment['total_records']:,}")
    print(f"🎯 Quality Content: {quality_assessment['substantial_content']:,}")
    print(f"🔍 Vector Indexes: {len(created_indexes)}")
    print(f"🧪 Validation Queries: {len(validation_results)}")

    if validation_results:
        avg_relevance = sum(r['relevance_score'] for r in validation_results) / len(validation_results)
        print(f"📈 Average Search Relevance: {avg_relevance:.1f}%")

    print("🚀 Ready for intelligent search implementation!")