...
# MAGIC %md
# MAGIC # ðŸš€ Hybrid Working Solution - Severity Classification Agent
# MAGIC
# MAGIC âœ… Combines **stable streaming ingestion** with **minimal AgentBricks schema**  
# MAGIC âœ… Ensures classification results are written (expected 15/15)  
# MAGIC âœ… Future-proof: can be extended with AgentBricks features
...
# Simplified severity classification table
# spark.sql(f"""
...
# MAGIC %md
# MAGIC ## 5. Define Severity Classifier

# COMMAND ----------

def classify_log(log_content: str):
...
table_path = spark.sql(f"DESCRIBE DETAIL {SEVERITY_TABLE}").collect()[0]['location']
print("Severity table path:", table_path)


# COMMAND ----------

# MAGIC %md
# MAGIC ## 6. Define Batch Processor
...
def process_batch(batch_df, batch_id):
    try:
        ...
        classified_df.write \
            .format("delta") \
            .mode("append") \
            .save(table_path)   # âœ… write directly to physical path
...
# MAGIC %md
# MAGIC ## 7. Start Streams (Dual: Raw + Severity)
...
raw_stream = (spark.readStream
    ...
    .withColumn("file_path", col("_metadata.file_path"))   # âœ… UC-safe replacement
...
# MAGIC %md
# MAGIC ## 8. Monitor for 60s

# COMMAND ----------

for i in range(12):
    time.sleep(5)
    count = spark.table(SEVERITY_TABLE).count()
    print(f"â³ [{i*5}s] Severity count = {count}")
...
classified_df.show(15, truncate=False)

# write once (non-streaming) into the severity table
classified_df.write.format("delta").mode("append").saveAsTable(SEVERITY_TABLE)
