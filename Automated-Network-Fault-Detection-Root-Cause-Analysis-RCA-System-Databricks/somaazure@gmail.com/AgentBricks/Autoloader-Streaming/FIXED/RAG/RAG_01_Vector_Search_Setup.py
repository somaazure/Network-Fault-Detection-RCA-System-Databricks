...
# MAGIC %md
# MAGIC # ðŸ” RAG Implementation - Vector Search Setup
# MAGIC
...
print("ðŸ” RAG Implementation - Vector Search Setup")
print("=" * 70)
...
def create_vector_search_endpoint():
    ...
    try:
        ...
        # Create new endpoint
        print(f"ðŸ”§ Creating Vector Search endpoint: {VS_ENDPOINT_NAME}")
...
def prepare_rca_data_for_vectorization():
    ...
    print("ðŸ“Š Preparing RCA data for vectorization...")
    ...
    try:
        ...
        print(f"ðŸ“Š Data quality retention: {(final_count/total_records)*100:.1f}%")

        # Show sample of prepared data
        print("\nðŸ” Sample prepared data:")
        enhanced_rca_df.select("id", "keywords", "search_content").limit(2).show(truncate=False)

        return enhanced_rca_df

    except Exception as e:
        print(f"âŒ Error preparing RCA data: {str(e)}")
        return None
...
if prepared_rca_df is not None:
    print(f"ðŸ’¾ Creating optimized RCA table: {OPTIMIZED_RCA_TABLE}")
...
def create_vector_search_index():
    ...
    if not endpoint_created or prepared_rca_df is None:
        print("âŒ Prerequisites not met for vector index creation")
        return False
    ...
    try:
        ...
        print(f"ðŸ”§ Creating vector search index: {VS_INDEX_NAME}")
        ...
        try:
            ...
            print(f"ðŸ“Š Index status: {'Ready' if status else 'Initializing (will be ready shortly)'}")
        except Exception as status_error:
            print(f"âš ï¸ Could not check index status: {str(status_error)}")
            print("ðŸ’¡ Index may still be initializing - this is normal")

        print("ðŸŽ¯ Vector search index created and will be ready shortly!")
        return True

    except Exception as e:
        print(f"âŒ Error creating vector search index: {str(e)}")
        print("ðŸ’¡ Tip: Make sure the endpoint is running and the table exists")
        return False
...
def sync_vector_index():
    ...
    if not index_created:
        print("âŒ Vector index not available for sync")
        return False

    try:
        print("ðŸ”„ Checking vector index sync status...")
        ...
        try:
            if hasattr(index_info, 'status'):
                print(f"ðŸ“Š Index status: {index_info.status}")
            elif hasattr(index_info, 'describe'):
                index_desc = index_info.describe()
                print(f"ðŸ“Š Index description available")
            else:
                print(f"ðŸ“Š Index object type: {type(index_info)}")
                print(f"ðŸ“Š Index available attributes: {[attr for attr in dir(index_info) if not attr.startswith('_')]}")
        except Exception as attr_error:
            print(f"ðŸ“Š Index info retrieved (details not accessible): {str(attr_error)}")

        print("ðŸ’¡ With TRIGGERED pipeline and Change Data Feed, sync happens automatically")
        return True

    except Exception as e:
        print(f"âŒ Error checking vector index: {str(e)}")
        print("ðŸ’¡ Index may still be initializing - this is normal for new indexes")
        return False
...
def test_vector_search():
    ...
    if not index_created:
        print("âŒ Vector index not available for testing")
        return
    ...
    try:
        ...
        print(f"ðŸ” Quick test query: '{test_query}'")
        ...
        if results and isinstance(results, dict) and 'result' in results:
            ...
            if data_array and len(data_array) > 0:
                ...
                print(f"   ðŸŽ¯ Top match: {root_cause} - {operation} (Priority: {priority})")
                print("   ðŸŽ‰ Vector Search is working correctly!")
            else:
        ...
        print(f"âŒ Vector search test failed: {str(e)}")
        print("ðŸ’¡ Index may still be initializing - try again in a few minutes")

    print("ðŸŽ¯ Vector search test completed")
...
print("ðŸ“‹ VECTOR SEARCH SETUP SUMMARY")
print("=" * 70)
print(f"ðŸŽ¯ Endpoint Name: {VS_ENDPOINT_NAME}")
print(f"ðŸ“Š Index Name: {VS_INDEX_NAME}")
print(f"ðŸ—ƒï¸ Source Table: {OPTIMIZED_RCA_TABLE}")
...
print(f"\nðŸŽ¯ RAG System Ready: {'âœ… YES' if all_ready else 'âŒ NO - Check failed steps'}")

if all_ready:
    print("ðŸ’¡ Next step: Implement RAG query interface")
else:
    print("ðŸ’¡ Fix the failed steps before proceeding to RAG implementation")
...
print("\nðŸ“„ Vector search configuration saved for RAG implementation")
