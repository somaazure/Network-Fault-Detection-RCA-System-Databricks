...
# MAGIC %md
# MAGIC # ðŸ“Š Pipeline Data Quality Check & Monitoring
# MAGIC
...
print("ðŸ“Š Pipeline Data Quality Check & Monitoring")
print("=" * 70)
...
def get_table_stats(table_name):
    ...
    try:
        ...
        return {
            ...
            "status": "âœ…" if total_count > 0 else "âŒ"
        }
...
print("ðŸ” PIPELINE DATA QUALITY ASSESSMENT")
print("=" * 70)
...
for table_name, description in TABLES.items():
    ...
    if "error" in stats:
        print(f"âŒ Error accessing table: {stats['error']}")
    else:
        print(f"ðŸ“Š Total Records: {stats['total_records']:,}")
        print(f"ðŸ• Recent Records (last hour): {stats['recent_records']:,}")
        print(f"â° Latest Timestamp: {stats['latest_timestamp']}")

        if stats['freshness_minutes'] is not None:
            print(f"ðŸ”„ Data Freshness: {stats['freshness_minutes']:.1f} minutes ago")
...
print("=" * 70)
print(f"ðŸ—ƒï¸  Healthy Tables: {healthy_tables}/{len(TABLES)}")
print(f"ðŸ“Š Total Pipeline Records: {total_pipeline_records:,}")
print(f"ðŸ“‹ Average Records per Table: {total_pipeline_records/len(TABLES):.1f}")
...
print("ðŸ”— DATA LINEAGE VALIDATION")
print("=" * 70)
...
def validate_data_lineage():
    ...
    try:
        ...
        print(f"\nðŸ”— Data Lineage Health: {'âœ… HEALTHY' if all_healthy else 'âš ï¸ ISSUES DETECTED'}")

        return lineage_results

    except Exception as e:
        print(f"âŒ Error validating data lineage: {str(e)}")
        return {}
...
print("ðŸŽ¯ RCA CONTENT QUALITY CHECK (RAG PREPARATION)")
print("=" * 70)
...
try:
    ...
    if rca_count > 0:
        print(f"ðŸ“Š Total RCA Reports: {rca_count:,}")
        ...
        print(f"ðŸ“ Average Content Length: {avg_length:.0f} characters")
        print(f"ðŸ“ Content Length Range: {min_length} - {max_length} characters")
        ...
        print(f"\nðŸŽ¯ RAG Readiness: {'âœ… READY' if rag_readiness else 'âš ï¸ NEEDS IMPROVEMENT'}")

    else:
        print("âŒ No RCA reports found")

except Exception as e:
    print(f"âŒ Error checking RCA content: {str(e)}")
...
try:
    ...
    if severity_df.count() > 0 and workflow_df.count() > 0:
        ...
        if processing_duration > 0:
            ...
            print(f"ðŸ“ˆ Pipeline Throughput: {records_per_second:.2f} records/second")

        print(f"ðŸ• First Severity Classification: {severity_times['first_severity']}")
        print(f"ðŸ• Last Workflow Completion: {workflow_times['last_workflow']}")

except Exception as e:
    print(f"âŒ Error calculating performance metrics: {str(e)}")
...
print("ðŸ“‹ FINAL PIPELINE HEALTH REPORT")
print("=" * 70)

pipeline_health_score = (healthy_tables / len(TABLES)) * 100
print(f"ðŸŽ¯ Pipeline Health Score: {pipeline_health_score:.1f}%")
...
if pipeline_health_score == 100:
    ...
    print("âŒ CRITICAL: Multiple agents failing")
...
print(f"\nðŸ“„ Pipeline report generated at: {datetime.now()}")

# COMMAND ----------

# Optional: Display tables for manual inspection
print("ðŸ” TABLE INSPECTION (First 5 rows of each table)")
print("=" * 70)

for table_name, description in TABLES.items():
    try:
        print(f"\nðŸ“‹ {description} ({table_name})")
        df = spark.table(f"{CATALOG_NAME}.{SCHEMA_NAME}.{table_name}")
        df.limit(5).display()
    except Exception as e:
        print(f"âŒ Cannot display {table_name}: {str(e)}")
