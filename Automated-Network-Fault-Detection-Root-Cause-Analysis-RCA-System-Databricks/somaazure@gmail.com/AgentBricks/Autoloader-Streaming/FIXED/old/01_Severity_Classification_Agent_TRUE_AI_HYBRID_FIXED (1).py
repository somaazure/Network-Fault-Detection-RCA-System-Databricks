...
def classify_with_fm(log_content: str) -> dict:
    ...
    try:
        response = client.predict(
            ...
            inputs={
                "messages": [
                    {"role": "system", "content": "You are an expert incident classifier. Respond with ONLY the severity level: P1, P2, P3, or INFO"},
                    {"role": "user", "content": f"Classify this log severity: {log_content}"}
                ],
                "temperature": 0.1,
                "max_tokens": 10  # âœ… Force shorter responses
            }
        )
        ...
        if prediction:
            ...
            # âœ… ROBUST PARSING: Extract P1, P2, P3, INFO from verbose text
            prediction_upper = prediction.upper()
            severity = "INFO"  # Default
            confidence = 0.8   # Default

            if "P1" in prediction_upper or "PRIORITY 1" in prediction_upper or "CRITICAL" in prediction_upper:
                severity = "P1"
                confidence = 0.90
            elif "P2" in prediction_upper or "PRIORITY 2" in prediction_upper or "HIGH" in prediction_upper:
                severity = "P2"
                confidence = 0.85
            elif "P3" in prediction_upper or "PRIORITY 3" in prediction_upper or "WARN" in prediction_upper:
                severity = "P3"
                confidence = 0.80
            else:
                severity = "INFO"
            ...
            return {
                ...
                "reasoning": f"FM classified {severity} from response: {prediction[:50]}..."
            }
...
def hybrid_classify(log_content: str) -> dict:
    ...
    if fm_res.get("success") and fm_res["confidence"] >= 0.8:
        return fm_res
...
# from pyspark.sql.functions import col# Start raw ingestion
