...
try:
    ...
    # IMMEDIATE TEST: Try a simple FM call right now
    print("ðŸ§ª Testing FM with simple call...")
    test_response = client.predict(
        endpoint=FOUNDATION_MODEL_NAME,
        inputs={
            "messages": [{"role": "user", "content": "Classify: CRITICAL system down. Answer with just P1, P2, P3, or INFO."}],
            "temperature": 0.1,
            "max_tokens": 5
        }
    )
    print(f"âœ… FM test successful! Response keys: {list(test_response.keys())}")
    
    # Extract prediction from test
    if "choices" in test_response:
        test_pred = test_response["choices"][0]["message"]["content"].strip()
        print(f"âœ… FM test prediction: '{test_pred}'")
    elif "predictions" in test_response:
        test_pred = test_response["predictions"][0]["candidates"][0]["text"].strip()
        print(f"âœ… FM test prediction: '{test_pred}'")
    
except Exception as e:
    client = None
    AI_ENABLED = False
    print(f"âš ï¸ FM client or test failed: {e}")
    print(f"ðŸ¤– AI_ENABLED = {AI_ENABLED}")
...
# Replace classify_with_fm with this robust version
def classify_with_fm(log_content: str, max_retries=2, retry_delay=1.0) -> dict:
    """
    Calls the foundation model robustly and returns:
    { success: bool, severity: "P1/P2/P3/INFO", confidence: float, method, reasoning }
    """
    if not AI_ENABLED or client is None:
        return {"success": False}

    # Strong instruction to model to respond strictly in JSON if possible
    prompt_system = "You are a concise incident classifier. Respond ONLY in one line JSON: {\"severity\":\"P1|P2|P3|INFO\",\"confidence\":0.0}"
    prompt_user = f"Log: {log_content}\nReturn JSON only."
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # DEBUG: print the raw response keys so we can see shape during runs
            print(f"ðŸ“¨ FM raw response (keys): {list(response.keys())}")
            ...
            if "choices" in response:
                ...
                if isinstance(p, dict) and "candidates" in p:
                    text = p["candidates"][0].get("text", "").strip()
            ...
            print(f"ðŸ”Ž FM text: {text}")

            # 1) Try parse JSON if model returned JSON
            try:
                import json
                parsed = json.loads(text)
                sev = parsed.get("severity", "").upper()
                conf = float(parsed.get("confidence", 0.0))
                if sev in ("P1","P2","P3","INFO") and 0.0 <= conf <= 1.0:
                    return {"success": True, "severity": sev, "confidence": conf,
                            "method": "ai_foundation_model", "reasoning": "parsed_json"}
            except Exception:
                # not JSON â€” continue to regex parsing
                pass

            # 2) Regex: find first occurrence of P1/P2/P3/INFO
            text_up = text.upper()
            msev = re.search(r"\b(P1|P2|P3|INFO)\b", text_up)
            mconf = re.search(r"([0-9]\.\d+|\d?\.\d+|\d+)%?", text)  # crude confidence capture (e.g. 0.85 or 85%)
            sev = msev.group(1) if msev else None
            conf = None
            if mconf:
                raw_conf = mconf.group(1)
                try:
                    fc = float(raw_conf)
                    # if percentage like '85' treat as 0.85
                    if fc > 1.0:
                        conf = fc/100.0
                    else:
                        conf = fc
            ...
            # 3) If we only found severity but not confidence, assign default trustworthy score
            if sev and conf is None:
                conf = 0.85

            if sev:
                print(f"âœ… FM parsed via regex: severity={sev}, confidence={conf}")
                return {"success": True, "severity": sev, "confidence": float(conf if conf is not None else 0.85),
                        "method": "ai_foundation_model", "reasoning": "parsed_regex"}

            # If we reach here, parsing failed -> treat as non-success and optionally retry
            print("âŒ FM parsing failed; will retry if attempts remain.")
        except Exception as e:
            print(f"âš ï¸ FM call exception (attempt {attempt}): {e}")
...
# Replace hybrid_classify with this explicit decision logic (with debug logs)
def hybrid_classify(log_content: str, ai_threshold=0.75) -> dict:
    """
    Use FM when parsed and confident >= ai_threshold, else fallback to rules.
    """
    print(f"ðŸ”„ HYBRID_CLASSIFY for: {log_content[:60]}...")
    fm = classify_with_fm(log_content)
    print(f"ðŸ” FM result: success={fm.get('success')}, severity={fm.get('severity')}, confidence={fm.get('confidence')}")
    if fm.get("success") and fm.get("confidence",0.0) >= ai_threshold:
        print(f"âœ… Choosing FM result: {fm['severity']} (conf {fm['confidence']:.2f})")
        return fm
    # else fallback to rules (always returns valid dict with confidence)
    rb = classify_with_rules(log_content)
    print(f"âš ï¸ Falling back to rules: {rb['severity']} (conf {rb['confidence']:.2f})")
    return rb
