...
def classify_with_fm(log_content: str) -> dict:
    """Working Foundation Model classification using proven logic"""
    if not AI_ENABLED:
        print("ðŸš« AI_ENABLED is False - skipping FM call")
        return {"success": False}
    print(f"ðŸ¤– WORKING FM CALL for: {log_content[:50]}...")
    ...
    try:
        ...
        # Extract prediction (we know format is "choices" from test)
        prediction = response["choices"][0]["message"]["content"].strip()
        print(f"ðŸ“¨ FM raw response: '{prediction[:100]}...'")
        
        # Parse severity from verbose response using PROVEN logic
        ...
        if "P1" in prediction_upper:
            ...
            severity = "P3"
        elif "INFO" in prediction_upper:
            severity = "INFO"
            
        print(f"âœ… WORKING PARSER - Extracted severity: {severity}")
        return {
            "success": True,
            "severity": severity,
...
def hybrid_classify(log_content: str) -> dict:
    ...
    if fm_res.get("success"):
        print(f"ðŸŽ‰ SUCCESS! Using AI classification: {fm_res.get('severity')} (confidence: {fm_res.get('confidence')})")
        return fm_res
    else:
        print(f"âš ï¸ FM failed, using rule-based fallback")
