...
def setup_network_ops_tables():
    ...
    try:
        ...
        try:
            ...
            print("âœ… Existing tables dropped to avoid schema conflicts")
        except:
        ...
        print("âœ… Network Operations Unity Catalog tables setup completed (FIXED VERSION)")
        return True
...
# MAGIC %md
# MAGIC ## ðŸ¤– Network Operations Agent Core Implementation
...
class DatabricksNetworkOpsAgent:
    ...
    def __init__(self):
        ...
        logger.info("Network Operations Agent initialized in simulation mode")
    ...
    def _perform_safety_checks(self, operation_type: OperationType, parameters: Dict[str, str]) -> Tuple[bool, List[str]]:
        ...
        for check in safety_checks:
            ...
            if check == "node_connectivity":
                ...
                safety_results.append(f"Node connectivity check: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
                    
            elif check == "backup_availability":
                passed = random.random() > 0.1  # 90% pass rate
                safety_results.append(f"Backup availability check: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
                    
            elif check == "neighbor_capacity":
                passed = random.random() > 0.15  # 85% pass rate
                safety_results.append(f"Neighbor capacity check: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
                    
            elif check == "traffic_patterns":
                passed = random.random() > 0.08  # 92% pass rate
                safety_results.append(f"Traffic patterns check: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
                    
            elif check == "current_qos_state":
                passed = random.random() > 0.05  # 95% pass rate
                safety_results.append(f"Current QoS state check: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
                    
            elif check == "resource_availability":
                passed = random.random() > 0.2  # 80% pass rate
                safety_results.append(f"Resource availability check: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
                ...
                passed = random.random() > 0.1  # 90% pass rate
                safety_results.append(f"{check}: {'âœ… PASS' if passed else 'âŒ FAIL'}")
                if not passed:
                    all_passed = False
    ...
    def execute_network_operation(self, operation_input: OperationInput) -> OperationResult:
        ...
        try:
            ...
            logger.info(f"âœ… Operation executed: {operation_input.operation_type.value}")
            logger.info(f"   â€¢ Operation ID: {operation_id}")
            logger.info(f"   â€¢ Success: {success}")
            logger.info(f"   â€¢ Duration: {execution_duration:.2f}s")
            logger.info(f"   â€¢ Affected Components: {len(affected_components)}")
    ...
    def save_operation_to_unity_catalog(self, operation_result: OperationResult, decision_id: int, operation_parameters: Dict[str, str]) -> bool:
        ...
        try:
            ...
            logger.info(f"âœ… Operation saved to Unity Catalog: {UC_TABLE_OPERATIONS}")
            return True
...
print("âœ… Network Operations Agent initialized successfully")

# COMMAND ----------

# MAGIC %md
# MAGIC ## ðŸ“‹ Process PENDING Decisions from Incident Manager
...
def process_pending_decisions():
    ...
    try:
        print("ðŸ” PROCESSING PENDING DECISIONS")
        print("=" * 70)
        ...
        if not pending_decisions:
            print("â„¹ï¸ No pending decisions found. Creating sample decisions for testing...")
            ...
            print("âœ… Sample decisions created for testing")
            pending_decisions = sample_df.collect()
        
        print(f"ðŸ“‹ Found {len(pending_decisions)} decisions to process:")
        ...
        for i, decision in enumerate(pending_decisions, 1):
            ...
            print("-" * 50)
            print(f"   â€¢ Severity: {decision.severity_classification}")
            print(f"   â€¢ Action: {decision.recommended_action}")
            print(f"   â€¢ Parameters: {decision.action_parameters}")
            ...
            try:
                ...
                # Execute operation
                print(f"   â€¢ Status: EXECUTING {decision.recommended_action}...")
                ...
                # Display results
                status_icon = "âœ… SUCCESS" if operation_result.success_indicator else "âŒ FAILED"
                print(f"   â€¢ Status: {operation_result.operation_status.value} {status_icon}")
                print(f"   â€¢ Duration: {operation_result.execution_duration_seconds:.2f}s")
                print(f"   â€¢ Components: {len(operation_result.affected_components)}")
                print(f"   â€¢ Unity Catalog: {'âœ… SAVED' if save_success else 'âŒ SAVE FAILED'}")
                print(f"   â€¢ Result: {operation_result.result_message}")
                
                results.append(operation_result)
                
            except Exception as e:
                logger.error(f"Failed to process decision {decision.incident_id}: {e}")
                print(f"   â€¢ Status: âŒ PROCESSING FAILED - {e}")
                continue
        
        # Generate summary
        print(f"\n{'='*70}")
        print("ðŸŽ¯ NETWORK OPERATIONS EXECUTION SUMMARY")
        print(f"{'='*70}")
        ...
        if results:
            ...
            print(f"   â€¢ Successful Operations: {successful_ops}")
            print(f"   â€¢ Success Rate: {success_rate:.1f}%")
            print(f"   â€¢ Average Duration: {avg_duration:.2f}s")
            ...
            for op_type, stats in operation_types.items():
                ...
                print(f"   â€¢ {op_type}: {stats['successful']}/{stats['total']} ({op_success_rate:.1f}%)")
            
            print(f"\nðŸ† Status: {'âœ… SUCCESS' if success_rate >= 80 else 'âš ï¸ PARTIAL SUCCESS' if success_rate >= 60 else 'âŒ NEEDS ATTENTION'}")
            
        return True
        
    except Exception as e:
        logger.error(f"Failed to process pending decisions: {e}")
        print(f"âŒ Processing failed: {e}")
        return False

# Execute processing
processing_success = process_pending_decisions()
print(f"\nðŸŽ¯ Network Operations Processing: {'âœ… COMPLETED' if processing_success else 'âŒ FAILED'}")
...
def verify_network_operations_data():
    ...
    try:
        ...
        if operations_count > 0:
            ...
            print(f"\nðŸ”’ Safety & Compliance Metrics:")
            print(f"   â€¢ Safety Checks Passed: {safety_stats['safety_passed']}/{safety_stats['total_ops']} ({safety_rate:.1f}%)")
            print(f"   â€¢ Simulated Operations: {safety_stats['simulated_ops']}/{safety_stats['total_ops']} ({simulation_rate:.1f}%)")
...
# MAGIC %md
# MAGIC ## ðŸŽ‰ Network Operations Agent Summary
# MAGIC
