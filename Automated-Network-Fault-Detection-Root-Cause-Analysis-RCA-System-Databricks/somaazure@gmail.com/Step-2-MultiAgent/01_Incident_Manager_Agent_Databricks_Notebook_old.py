...
# DATABRICKS_TOKEN = dbutils.secrets.get("databricks", "token") if dbutils.secrets.listScopes() else "YOUR_DATABRICKS_TOKEN"
...
The set of values is fixed â€” you canâ€™t accidentally reassign them.
"""
...
try:
    ...
    logger.info("âœ… Decisions saved successfully to Unity Catalog!")
except Exception as e:
    logger.error(f"Failed to save decision to Unity Catalog: {e}")
...
def verify_incident_decisions_data():
    ...
    try:
        ...
        if decisions_count > 0:
            ...
            print("\nðŸ“‹ Recent Incident Decisions:")
            for _, row in recent_decisions.iterrows():
                escalation = "ðŸš¨" if row['escalation_required'] else "âœ…"
            ...
            for _, row in action_stats.iterrows():
                print(f"   â€¢ {row['recommended_action']}: {row['count']} decisions "
...
# MAGIC %md
# MAGIC ## ðŸš€ Production Integration Functions
...
def batch_process_incidents(incidents: List[IncidentInput]) -> List[IncidentDecision]:
    ...
    decisions = []
    print(f"ðŸ”„ Processing {len(incidents)} incidents in batch...")
    ...
    print(f"âœ… Batch processing completed: {len(decisions)}/{len(incidents)} successful")
    return decisions
...
def create_incident_decision_api():
    ...
    print("ðŸš€ Incident Decision API Specification:")
...
# MAGIC %md
# MAGIC ## ðŸ“ˆ Performance Analytics Dashboard
...
def generate_decision_performance_dashboard():
    ...
    try:
        print("ðŸ“ˆ Incident Manager Performance Dashboard:")
        print("=" * 60)
        ...
        if not accuracy_df.empty:
            print("ðŸ“Š 7-Day Decision Performance:")
            for _, row in accuracy_df.iterrows():
                escalation_rate = (row['escalations'] / row['total_decisions']) * 100
                print(f"   â€¢ {row['recommended_action']}: {row['total_decisions']} decisions | "
        ...
        if not priority_df.empty:
            print(f"\nðŸ“Š Priority Distribution (7 days):")
            for _, row in priority_df.iterrows():
                print(f"   â€¢ {row['priority_level']} - {row['severity_classification']}: {row['count']} incidents")
...
# MAGIC %md
# MAGIC ## ðŸŽ‰ Incident Manager Agent Summary
# MAGIC
# MAGIC ### âœ… **Completed Features:**
